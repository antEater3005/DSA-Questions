...
void Preorder(node *root) //This a recursive method.
{
    if (root == NULL)
        return;
    cout << root->data << " ";
...
void Postorder(node *root) //This a recursive method.
{
    if (root == NULL)
        return;
    Postorder(root->left);
...
void Inorder(node *root) //This a recursive method.
{
    if (root == NULL)
        return;
    Inorder(root->left);
...
// Below function finds the Lowest Commom Ancestor in the binary tree
...
// method 2
void leftView(node *root, vector<int> &LeftView, int level)
{
    if (!root)
        return;

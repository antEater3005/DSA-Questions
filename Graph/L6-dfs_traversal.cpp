...
// DFS approach
void topologicalSort_DFS_util(vector<vector<int>> adj, stack<int> &topo, vector<bool> &visited, int node)
{
    visited[node] = true;
    for (int a : adj[node])
    {
        if (!visited[a])
            topologicalSort_DFS_util(adj, topo, visited, a);
    }
    topo.push(node);
}
vector<int> topologicalSort_DFS(vector<vector<int>> adj, int n)
{
    ...
    {
        if (!visited[i])
            topologicalSort_DFS_util(adj, topo, visited, i);
    }
    vector<int> ans;
    while (!topo.empty())
    {
...
vector<int> dijkstra(int n, vector<vector<int>> adj[], int source)
{
    ...
    while (!pq.empty())
    {
        ...
        for (auto a : adj[temp.second])
        {
            if (temp.first + a[1] < distance[a[0]])
            {
                distance[a[0]] = temp.first + a[1];
                pq.push({distance[a[0]], a[0]});
            }
        }
    }
    return distance;
 ...
 * The listed edeges are traversed and checked if the parent and child have same parent in our disjoint set if they have then the edge is not included but if its not in our disjoint set its added into the MST;
 */
...
/**Bridge in Graph:: bridges in  a graph are the edges the removal of which increases the total number of the given graph; */
